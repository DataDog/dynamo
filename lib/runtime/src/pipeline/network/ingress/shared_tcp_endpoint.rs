...
impl SharedTcpServer {
    ...
    /// mirroring the call-home server's `build_tls_acceptor`. When a client CA
    /// is provided, mTLS is enforced (clients must present a trusted cert).
        ...
        match (cert, key) {
            (Some(c), Some(k)) => {
                ...
                if client_ca.is_some() {
                    tracing::info!("TCP request plane: mTLS enabled (client certificates required)");
                } else {
...
mod tests {
    ...
    fn request_plane_tls_reads_client_ca_path() {
        ...
        let error = SharedTcpServer::request_plane_tls_acceptor(
            Some(cert.path()),
            Some(key.path()),
            Some(std::path::Path::new("/nonexistent/request-plane-client-ca.pem")),
        )
        .err()
