...
[Operator TLS](../../kubernetes/tls.md).
...
- Server and mTLS client identity certificate/key files are checked for changes
  at most once every 30 seconds when a new handshake occurs. A valid rotated
  pair is loaded without restarting the process; a partial or invalid rotation
  keeps the last valid identity and retries on the next handshake after a short
  backoff (1s) instead of waiting the full check interval. CA bundle changes
